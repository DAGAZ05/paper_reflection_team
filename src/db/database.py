...
class DatabaseManager:
            ...
            async with self.acquire() as conn:
                if dedup_by_rule:
                    ...
                    query = f"""
                        ...
                        WHERE rn = 1
                        ORDER BY paper_id, agent_code, audit_time
                    """
                    ...
                    if paper_id:
                        query = """
                            ...
                            FROM agent_audit_result
                            WHERE paper_id = $1
                            ORDER BY agent_code, audit_time
                        """
                ...
                for row in rows:
                    ...
                    results.append({
                        ...
                        "audit_suggestion": row["audit_suggestion"],
                        "audit_time": row["audit_time"],
                        "result_json": result_json,
                    })

                logger.info(f"从agent_audit_result表读取了{len(results)}条审计结果" +
                           (f"（已按rule_id去重）" if dedup_by_rule else ""))
                return results
    ...
    async def check_paper_has_all_agents(self, paper_id: str) -> Tuple[bool, List[str]]:
        """
        检查指定paper_id是否同时有4个审计组(FMT/REF/EXP/LOG)的结果
        ...
        try:
            async with self.acquire() as conn:
                ...
                """
                rows = await conn.fetch(query, paper_id)
                existing_codes = {row["agent_code"] for row in rows}
    ...
    async def check_needs_reevaluation(self, paper_id: str) -> bool:
        """
        检查是否需要重新评估：
        如果agent_audit_result中有audit_time在reflect_agent_verdict的verdict_time之后的记录，
        则需要重新评估。
        ...
        try:
            ...
            async with self.acquire() as conn:
                query = """
                    SELECT COUNT(*) as cnt
                    FROM agent_audit_result
                    WHERE paper_id = $1 AND audit_time > $2
                """
